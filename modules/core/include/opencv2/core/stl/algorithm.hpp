...
auto all_of(std::tuple<Args...> tpl, index_sequence<Is...>)
...
auto all_of(Args&&... args)
  ...
  if (__iterators__replaceable(std::forward<Args>(args)...)) {
    return all_of(
        make_tpl_replaced(std::forward<Args>(args)...),
...
/// replaced as pointers
template <typename... Args, std::size_t... Is>
auto any_of(std::tuple<Args...> tpl, index_sequence<Is...>)
...
/// replaced as pointers
template <typename... Args, std::size_t... Is>
auto none_of(std::tuple<Args...> tpl, index_sequence<Is...>)
...
/// replaced as pointers
template <typename... Args, std::size_t... Is>
auto count_if(std::tuple<Args...> tpl, index_sequence<Is...>)
...
auto find(const ReturnType &, beginIt && begin,std::tuple<Args...> tpl, index_sequence<Is...>)
    -> ReturnType {
...
template <typename ReturnType,  typename beginIt, typename... Args, std::size_t... Is,
    enable_if_t<!std::is_base_of<cv::MatConstIterator, ReturnType>::value,bool> = true>
auto find(const ReturnType &, beginIt && , std::tuple<Args...> tpl, index_sequence<Is...>)
  -> ReturnType {
