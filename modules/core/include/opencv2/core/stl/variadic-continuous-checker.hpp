...
// Thanks to
// https://stackoverflow.com/questions/26855322/how-do-i-get-the-index-of-a-type-matching-some-predicate
template <template <class T> class, typename, long = 0> struct find_if;

template <template <class T> class Pred, typename T, long pos, typename... tail>
struct find_if<Pred, std::tuple<T, tail...>, pos>
    : std::conditional<Pred<T>::value, std::integral_constant<int64_t, pos>,
...
template <template <class, class> class T, class U> struct bind {
  template <class X> using first = T<U, X>;
  template <class X> using second = T<X, U>;
};

/// Helper function: Return the index of the first cv::MatConstIterator derived
/// type. Extends the tuple with an iterator such that it will always be found,
/// otherwise compilation fails
template <typename... Args> constexpr size_t __get_first_cv_it_index() {
  return find_if<bind<std::is_base_of, cv::MatConstIterator>::first,
                 std::tuple<Args..., cv::MatConstIterator>>::value ==
                 sizeof...(Args)
             ? 0
             : find_if<bind<std::is_base_of, cv::MatConstIterator>::first,
                       std::tuple<Args..., cv::MatConstIterator>>::value;
}

/// Helper function: Return the index of second last element.
template <typename... Args> constexpr size_t __get_second_last_index() {
