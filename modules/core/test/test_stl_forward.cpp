...
TEST_F(CORE_stl_forward, iterators_replacable)
{
    ...
    EXPECT_TRUE(cv::experimental::__iterators__replaceable(mat.begin<int>(), mat.end<int>(),[](int val){return val*2;}));

    //Should be false because there is no cv::Mat iterator. Lambda doesn't have an effect
    EXPECT_FALSE(cv::experimental::__iterators__replaceable(intVec.begin(), intVec.end(),[](int val){return val*2;}));

    //Should be true because there is only one cv::Mat iterator of a contiguous iterator. Lambda doesn't have an effect
    EXPECT_TRUE(cv::experimental::__iterators__replaceable(intVec.begin(), intVec.end(), mat.begin<int>(),[](int val){return val*2;}));

    ///////////////////////////////////Submatrix Tests/////////////////////////////////
    //Test only one "span" where we can't replace iterators (not contiguous)
    EXPECT_FALSE(cv::experimental::__iterators__replaceable(subMat.begin(), subMat.end()));

    //Should be false because there is a cv::Mat iterator of a non-contiguous matrix
    EXPECT_FALSE(cv::experimental::__iterators__replaceable(intVec.begin(), intVec.end(), subMat.begin()));
...
TEST_F(CORE_stl_forward, DISABLED_iterators_replacable_reverse_iterator)
{
    ///////////////////////////////////Contiguous Tests/////////////////////////////////
    //Test only one "span" where we can replace iterators. Lambda doesn't have an effect
    EXPECT_TRUE(cv::experimental::__iterators__replaceable(mat.rbegin<uchar>(), mat.rend<uchar>(),[](uchar val){return val*2;}));

    //Should be false because there is no cv::Mat iterator. Lambda doesn't have an effect
    EXPECT_FALSE(cv::experimental::__iterators__replaceable(intVec.rbegin(), intVec.rend(),[](uchar val){return val*2;}));

    //Should be true because there is no cv::Mat iterator of a non contiguous iterator. Lambda doesn't have an effect
    EXPECT_TRUE(cv::experimental::__iterators__replaceable(intVec.rbegin(), intVec.rend(), mat.rbegin<uchar>(),[](uchar val){return val*2;}));

    ///////////////////////////////////Submatrix Tests/////////////////////////////////
    //Test only one "span" where we can't replace iterators (not contiguous)
    EXPECT_FALSE(cv::experimental::__iterators__replaceable(subMat.rbegin(), subMat.rend()));

    //Should be false because there is a cv::Mat iterator of a non-contiguous matrix
    EXPECT_FALSE(cv::experimental::__iterators__replaceable(intVec.begin(), intVec.end(), subMat.rbegin()));

    //Should be false because there is a cv::Mat iterator of a non-contiguous matrix
    EXPECT_FALSE(cv::experimental::__iterators__replaceable(intVec.rbegin(), intVec.end(), subMat.rbegin()));
}

TEST_F(CORE_stl_forward, tuple_replacer)
{
    //Simple example of only opencv iterators being replaced by their pointers
    auto itReplace= cv::experimental::make_tpl_replaced(mat_f.begin(), mat_f.end());
    ...
    auto itReplaceSub= cv::experimental::make_tpl_replaced(subMat.begin(), subMat.end());
    ...
    auto itReplaceMixed= cv::experimental::make_tpl_replaced(mat.begin<int>(), mat.end<int>(),intList.begin());
    ...
    auto itReplaceMixed_order_reversed= cv::experimental::make_tpl_replaced(intList.begin(),mat.begin<int>(), mat.end<int>());
    ...
    auto itReplaced_lambda= cv::experimental::make_tpl_replaced(intList.begin(),mat.begin<int>(), mat.end<int>(),[](int val){return 2*val;});
...
{
    static_assert(cv::experimental::__get_first_cv_it_index<decltype(mat.begin<int>())>() == 0, "Wrong index chosen");
    static_assert(cv::experimental::__get_first_cv_it_index<decltype(mat.begin<int>()),decltype(mat.end<int>())>() == 0, "Wrong index chosen");

    static_assert(cv::experimental::__get_first_cv_it_index<decltype(intVec.begin()), decltype(intVec.begin())>() == 0, "Wrong index chosen");
    static_assert(cv::experimental::__get_first_cv_it_index<decltype(intVec.begin()), decltype(intVec.begin()), decltype(mat.begin<int>())>() == 2, "Wrong index chosen");
    static_assert(cv::experimental::__get_first_cv_it_index<decltype(intVec.begin()), decltype(mat.begin<int>()), decltype(intVec.begin())>() == 1, "Wrong index chosen");
}
...
TEST_F(CORE_stl_forward, count_if_test)
{
    ...
    EXPECT_TRUE(cv::experimental::__iterators__replaceable(mat.begin<int>(), mat.end<int>(),lambda));
